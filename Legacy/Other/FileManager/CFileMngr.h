...
#define OUT_BUF_START_SIZE	 0x100 //!Cvet стартовый размер исходящего буфера
...
#define PT_PRO_CRIT		21	// !Deniska прототипы критов
...
#ifdef FM_USE_DAT_MANAGER //!Cvet
	int Init(char* data_path, char* master_path, char* critter_path); //!Cvet
#else
...
//полный путь к файлу
	...
	const char* GetPath(int PathType);
	//!Cvet -----------
	...
	int ParseLinesInt(char* fname, int PathType, vector<int>& lines); //!Cvet
